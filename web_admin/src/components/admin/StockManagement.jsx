...
export default function StockManagement({ masterData, setMasterData, selectedBranch, themeMode = 'dark' }) {
  ...
  const getMovementsList = () => {
    ...
    // Auto-stream HPP/Bahan Mentah entries from Laporan Keuangan into Stok Masuk
    ...
    allFinanceReports.forEach(f => {
      ...
      // Allow any report unless explicitly rejected or draft
      const isRejected = st === 'rejected' || st === 'ditolak' || st === 'draft';
      if (isRejected) return;

      const cleanDate = String(f.date || f.entry_date || f.created_at || new Date().toISOString()).substring(0, 10);
      const cogsItems = f.cogs_items || f.cogs_breakdown || f.pembelian_barang || f.items || [];
      const expenseRows = f.expense_rows || [];
      const expenseBreakdown = f.expenses_breakdown || f.expense_items || [];
      ...
      // 2. From expense_rows array (from Admin add/edit modal)
      if (!itemPushed && expenseRows && expenseRows.length > 0) {
        expenseRows.forEach((ex, idx) => {
          const cat = String(ex.category_type || ex.cost_group || '').toLowerCase();
          if (cat.includes('hpp') || cat.includes('cogs') || cat.includes('bahan')) {
            const autoId = `mov-fin-exrow-${f.id || f.report_no}-${idx}`;
            if (!existingIds.has(autoId)) {
              itemPushed = true;
              baseList.push({
                id: autoId,
                ...
                date: cleanDate,
                outlet_id: Number(f.outlet_id || f.branch_id || 1),
                created_by: f.author_name || f.cashier || f.cashier_name || 'Admin/Kasir (by Laporan Keuangan)',
                item_name: ex.item_name || ex.name || 'Bahan Mentah Dapur',
                supplier: 'Supplier Dapur HPP',
                qty: Number(ex.qty || 1),
                unit: ex.unit || 'kg',
                price_unit: Number(ex.price_per_unit || ex.price || 0),
                total_price: Number(ex.subtotal || ex.amount || 0),
                type_input: 'by laporan keuangan (ACC)',
                report_no: f.report_no || f.id,
                status: f.status || 'Done'
              });
      ...
      // 3. From expenses_breakdown array (from POS Kasir shift closing)
      if (!itemPushed && expenseBreakdown && expenseBreakdown.length > 0) {
        expenseBreakdown.forEach((ex, idx) => {
          const cat = String(ex.cost_group || ex.category || '').toLowerCase();
          if (cat.includes('cogs') || cat.includes('hpp') || cat.includes('bahan')) {
            const autoId = `mov-fin-ex-${f.id || f.report_no}-${idx}`;
            if (!existingIds.has(autoId)) {
              itemPushed = true;
              const rawName = String(ex.category || ex.name || 'Bahan Mentah Dapur');
              const itemName = rawName.replace(/\[HPP Dapur\]\s*/i, '').trim();

              // Parse qty & price from notes if available (e.g. "5 kg @ Rp 50.000")
              let parsedQty = 1;
              let parsedUnit = 'kg';
              if (ex.notes) {
                const match = String(ex.notes).match(/^([\d\.,]+)\s*(\w+)/);
                if (match) {
                  parsedQty = Number(match[1].replace(',', '.')) || 1;
                  parsedUnit = match[2] || 'kg';
                }
              }

              const totalAmt = Number(ex.amount || ex.subtotal || 0);
              const priceUnit = parsedQty > 0 ? totalAmt / parsedQty : totalAmt;
      ...
      // 4. Direct Fallback if cogs_expense > 0 or total_expense > 0 and no item pushed yet
      const cogsVal = Number(f.cogs_expense || 0);
      if (!itemPushed && cogsVal > 0) {
  ...
  const handleSendToMobileAPK = (recordName, recordObj = null) => {
    if (recordObj) {
      if (recordObj.status !== 'ACC' && recordObj.status !== 'Approved' && recordObj.status !== 'ok') {
        alert(`Laporan Stok Opname (${recordName}) harus disetujui (ACC) terlebih dahulu oleh Admin sebelum dapat dikirim ke Mobile APK!`);
        return;
      }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', background: T.pageBg, color: T.txtPrimary, transition: 'background 0.25s ease' }} className="animate-fade-in">
                ...
                <label key={key} style={{
                  ...
                  <span>
                    {key === 'date' ? 'Tanggal' : 
                     ...
                     key === 'systemQty' ? 'Stok Sistem' :
                     key === 'actualQty' ? 'Stok Fisik' :
                     ...
                     key === 'stokRusak' ? 'Stok Rusak' :
                     key === 'stokFisik' ? 'Sisa Stok Fisik' :
                     key === 'stokSistem' ? 'Sisa Stok Sistem' :
                     key === 'selisih' ? 'Selisih' :
                    ...
                    return paginatedMasuk.map(m => {
                      ...
                      return (
                        <tr key={m.id} style={{ borderBottom: `1px solid ${T.border}`, transition: 'background 0.15s' }} >
                          ...
                          {/* 2. NO LAPORAN */}
                          <td style={{ padding: '8px 10px' }}>
                            <div style={{ color: T.info, fontWeight: '800', fontSize: '0.72rem', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                              <span>{m.report_no || m.id}</span>
                              <Eye size={12} color={T.info} />
                            </div>
                            <div style={{ fontSize: '0.64rem', color: T.txtSecondary, marginTop: '2px' }}>
                          ...
                          <td style={{ padding: '8px 10px' }}>
                            <span style={{
                              ...
                              fontSize: '0.66rem',
                              fontWeight: '800',
                              background: isWebAdminInput ? 'rgba(56, 189, 248, 0.12)' : 'rgba(99, 102, 241, 0.12)',
                              color: isWebAdminInput ? T.info : T.info,
                              border: isWebAdminInput ? '1px solid rgba(56, 189, 248, 0.28)' : '1px solid rgba(99, 102, 241, 0.28)'
                            }}>
                              {isWebAdminInput ? 'Admin' : 'POS Kasir'}
                            </span>
                            <div style={{ fontSize: '0.64rem', color: T.txtSecondary, marginTop: '2px' }}>{m.created_by || 'Admin Logistik'}</div>
                          </td>
                          ...
                          <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                            <button
                              ...
                              style={{
                                padding: '4px 10px',
                                borderRadius: '6px',
                                background: isDone ? 'rgba(34, 197, 94, 0.12)' : 'rgba(245, 158, 11, 0.15)',
                              ...
                              title={isDone ? 'Status sudah Done (Disetujui)' : 'Klik untuk menyetujui log stok masuk ini'}
                            >
        ...
        {activeSubTab === 'stok_opname_system' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ overflowX: 'auto', border: `1px solid ${T.border}`, borderRadius: '10px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.86rem' }}>
                <thead>
                  <tr style={{ background: T.cardBg2, borderBottom: `1px solid ${T.border}`, color: T.txtSecondary, textAlign: 'left' }}>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '50px' }}>NO</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800' }}>NAMA ITEM / BAHAN BAKU</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '100px', textAlign: 'center' }}>SATUAN</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '120px', textAlign: 'right' }}>STOK AWAL</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '120px', textAlign: 'right', color: T.success }}>STOK MASUK</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '120px', textAlign: 'right', color: T.danger }}>STOK KELUAR</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '120px', textAlign: 'right', color: T.info }}>TRANSFER IN</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '120px', textAlign: 'right', color: T.danger }}>TRANSFER OUT</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '120px', textAlign: 'right', color: T.accentGold }}>STOK RUSAK</th>
                    <th style={{ padding: '8px 10px', fontWeight: '800', width: '160px', textAlign: 'right', color: T.info }}>SISA STOK SISTEM</th>
                  </tr>
                </thead>
                <tbody>
                  {(() => {
                    ...
                    return paginatedSystemRows.map((row, idx) => {
                      return (
                        <tr key={row.id || idx} style={{ borderBottom: `1px solid ${T.border}`, color: T.txtPrimary, transition: 'background 0.15s' }} >
                          <td style={{ padding: '8px 10px', color: T.txtSecondary, fontWeight: '700' }}>
                            {(currentPage - 1) * pageSize + idx + 1}
                          </td>

                          <td style={{ padding: '8px 10px' }}>
                            <strong style={{ color: T.txtPrimary, display: 'block', fontSize: '0.76rem' }}>
                              {row.itemName}
                            </strong>
                          </td>
                          ...
                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: T.txtPrimary }}>
                            {row.stokAwal} {row.unit}
                          </td>

                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: row.stokMasuk > 0 ? T.success : T.txtMuted }}>
                            {row.stokMasuk} {row.unit}
                          </td>

                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: row.stokKeluar > 0 ? T.danger : T.txtMuted }}>
                            {row.stokKeluar} {row.unit}
                          </td>

                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: row.transferIn > 0 ? T.info : T.txtMuted }}>
                            {row.transferIn} {row.unit}
                          </td>

                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: row.transferOut > 0 ? T.danger : T.txtMuted }}>
                            {row.transferOut} {row.unit}
                          </td>

                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: row.stokRusak > 0 ? T.accentGold : T.txtMuted }}>
                            {row.stokRusak} {row.unit}
                          </td>

                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: T.info, fontSize: '0.78rem' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.86rem' }}>
                <thead>
                  <tr style={{ background: T.cardBg2, borderBottom: `2px solid ${T.border}`, color: T.txtSecondary, textAlign: 'left' }}>
                    ...
                    <th style={{ padding: '14px 12px', fontWeight: '800', textAlign: 'right', color: T.danger, background: 'rgba(251,113,133,0.08)', whiteSpace: 'nowrap' }}>STOK RUSAK (-)</th>
                    <th style={{ padding: '14px 12px', fontWeight: '800', textAlign: 'right', color: T.warning, whiteSpace: 'nowrap' }}>STOK FISIK</th>
                ...
                <tbody>
                  {(() => {
                    ...
                    return paginatedOpname.map(op => {
                      ...
                      const sSistem = (op.stok_awal || 0) + (op.stok_masuk || 0) + (op.transfer_masuk || 0) - (currentStokKeluar + currentStokRusak + (op.transfer_keluar || 0));
                      const stokFisikVal = op.stok_fisik !== undefined ? op.stok_fisik : (op.stok_awal || 0);
                      const diffVal = stokFisikVal - sSistem;
                      ...
                      return (
                        <tr key={op.id} style={{ borderBottom: `1px solid ${T.border}`, color: T.txtPrimary, transition: 'background 0.15s' }} >
                          {/* 1. TANGGAL */}
                          <td style={{ padding: '8px 10px', color: T.txtPrimary, fontWeight: '600' }}>
                            <div>{op.date}</div>
                          ...
                          {/* 6. STOK RUSAK — otomatis dari sub-tab Stok Rusak outlet */}
                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: currentStokRusak > 0 ? T.danger : T.txtMuted, background: currentStokRusak > 0 ? 'rgba(251,113,133,0.06)' : 'transparent', whiteSpace: 'nowrap' }}>
                            {currentStokRusak > 0 ? `-${currentStokRusak} ${op.unit || 'kg'}` : `0 ${op.unit || 'kg'}`}
                            {currentStokRusak > 0 && (
                              <div style={{ fontSize: '0.60rem', color: T.txtMuted, marginTop: '2px' }}>auto dari laporan rusak</div>
                            )}
                          </td>

                          {/* 7. STOK FISIK */}
                          <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '800', color: T.warning, whiteSpace: 'nowrap' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 110 }}>
          <form onSubmit={handleSaveEditRecord} className="glass-card animate-fade-in" style={{ padding: '24px', width: '480px', border: `1px solid ${editType === 'masuk' ? T.info : editType === 'waste' ? T.danger : editType === 'opname' ? T.success : T.accentGold}`, display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: '900', color: T.txtPrimary, borderBottom: `1px solid ${T.border}`, paddingBottom: '12px' }}>
              {editType === 'masuk' ? 'Edit Record Logistik Masuk' : editType === 'waste' ? 'Edit Laporan Stok Rusak (Waste)' : editType === 'opname' ? 'Edit Audit Stock Opname' : 'Edit Transfer Stok Antarcabang'}
            </h3>
                                  ...
                                  <tr style={{ background: 'rgba(56, 189, 248, 0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                    <td colSpan={6} style={{ padding: '8px 10px' }}>
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <span style={{ fontSize: '0.74rem', color: T.accentGold, fontWeight: '800' }}>Nama Bahan Baku Baru:</span>
                                        <input
                                          type="text"
                                          required
          ...
          <div className="glass-card animate-fade-in" style={{
            ...
            <form onSubmit={handleSaveRusak} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '6px' }}>
                ...
                <button type="submit" className="btn-primary" style={{ padding: '8px 16px', fontSize: '0.8rem', background: T.danger, color: T.txtInverse, fontWeight: '800' }}>
          ...
          <div className="glass-card animate-fade-in" style={{ width: '100%', maxWidth: '580px', maxHeight: '90vh', overflowY: 'auto', padding: '24px', background: T.cardBg, border: `1px solid ${T.success}`, borderRadius: '18px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '8px', borderTop: `1px solid ${T.border}`, paddingTop: '14px' }}>
              ...
              <button
                ...
                style={{ padding: '9px 24px', background: `linear-gradient(135deg, ${T.success} 0%, #059669 100%)`, border: 'none', color: T.txtInverse, borderRadius: '8px', fontSize: '0.85rem', fontWeight: '900', cursor: 'pointer', boxShadow: '0 4px 14px rgba(52,211,153,0.4)' }}
              >
                Simpan Laporan
              </button>
            </div>
          ...
          <div className="glass-card animate-fade-in" style={{ padding: '24px', width: '100%', maxWidth: '520px', background: T.cardBg, border: `1px solid ${T.info}`, borderRadius: '12px' }}>
            ...
            <form onSubmit={handleSavePriceRecord} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ background: T.cardBg2, padding: '14px', borderRadius: '8px', border: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: '800', color: `${T.info}` }}>Input Harga Satuan (IDR) per Cabang Outlet:</span>
                {outlets.map(o => (
                      ...
                      <tbody>
                        {itemsList.map((row, i) => (
                          <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', color: T.txtPrimary }}>
                            ...
                            <td style={{ padding: '6px', color: T.danger }}>{row.damage_reason || row.reason || 'Lainnya'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          <div className="glass-card animate-fade-in" style={{ padding: '24px', width: '780px', maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${T.info}`, background: T.cardBg2, borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${T.border}`, paddingBottom: '14px' }}>
              <div>
                <h3 style={{ fontSize: '1.25rem', fontWeight: '900', color: T.txtPrimary, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span>{editingOutflowRecord ? `Edit Transaksi Penjualan (${salesNo})` : 'Tambah Transaksi Penjualan (Input Order POS Manual)'}</span>
                </h3>
                <p style={{ fontSize: '0.78rem', color: T.txtSecondary, margin: '4px 0 0 0' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  <thead>
                    <tr style={{ background: T.cardBg, color: T.txtPrimary, borderBottom: `1px solid ${T.border}`, textTransform: 'uppercase', fontSize: '0.72rem', fontWeight: '800' }}>
                      ...
                      <th style={{ padding: '10px 12px', width: '190px' }}>Varian Menu / Opsi</th>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', borderTop: `1px solid ${T.border}`, paddingTop: '16px' }}>
              ...
              <button
                ...
                className="btn-primary"
                style={{ padding: '10px 24px', fontSize: '0.85rem', background: `${T.info}`, color: T.txtInverse }}
              >
      ...
      {previewOutflowModalData && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 110 }}>
          <div className="glass-card animate-fade-in" style={{ padding: '24px', width: '680px', maxHeight: '85vh', overflowY: 'auto', border: `1px solid ${T.borderStrong}`, background: T.cardBg2, borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div className="glass-card animate-fade-in" style={{ padding: '24px', width: '940px', maxHeight: '90vh', overflowY: 'auto', border: `1px solid ${T.info}`, display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
              ...
              <button
                type="button"
                onClick={() => setPreviewOpnameReportModalData(null)}
                style={{ padding: '10px 24px', background: T.info, color: T.txtInverse, border: 'none', borderRadius: '8px', fontWeight: '900', fontSize: '0.85rem', cursor: 'pointer' }}
              >
                Selesai &amp; Tutup
              </button>
            </div>
