...
export default function App() {
  ...
  // Live polling & explicit refresh dari server VPS (Single Source of Truth)
  const fetchLatestFromServer = useCallback((force = false) => {
    // Guard: jangan fetch saat admin baru saja mutasi data lokal kecuali jika force=true
    if (!force) {
        ...
        if (serverData && typeof serverData === 'object' && !Array.isArray(serverData)) {
          ...
          setMasterData(prev => {
            ...
            const getItemKey = (item) => {
              ...
              if (item.receipt_no && String(item.receipt_no).trim() !== '') return String(item.receipt_no);
  ...
  // Polling interval
  useEffect(() => {
    setTimeout(() => fetchLatestFromServer(true), 300);
    const livePollTimer = setInterval(() => fetchLatestFromServer(false), 5000);
